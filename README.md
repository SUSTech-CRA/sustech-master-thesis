...
开发版仅供开发者和需要使用最新功能的高级 TeX 用户使用，不提供任何保证。开发版不含 `cls` 模板类文件，如需生成请参考[升级-开发版](#开发版)。
...
* 开发版：
  ...
  * [GitHub](https://github.com/SUSTech-CRA/sustech-master-thesis)：GitHub 上的源码仓库地址
  * [SUSTech-Git](https://mirrors.sustech.edu.cn/git/liziwl/sustech-master-thesis)：南方科技大学自建 Git 服务上的源码仓库地址

## 使用

### 编译前的建议

1. 在撰写论文时，我们不推荐使用原有的 `sustechthesis-example.tex` 这一名称。建议将其复制一份，改为其他的名字（如 `thesis.tex` 或 `main.tex`）。需要注意，如果使用了来自 `data` 目录中的 `tex` 文件，则重命名主文件后，其顶端的 `!TeX root` 选项也需要相应修改。
...
本项目已内置 VSCode 项目配置：
* `latexmk for sustechthesis`：使用 `latexmk` 生成学位论文示例文档 sustechthesis-example.pdf
...
下载发布版的 zip 包，复制其中的 `sustechthesis.*` 开头的文件，覆盖现有项目内的旧文件即可（必须包括 `dtx`、`ins`、`cls`）。

### 开发版

从 GitHub clone 项目源码或下载源码 zip 包，在项目目录下执行以下命令生成 `sustechthesis.cls` 等模板文件：
